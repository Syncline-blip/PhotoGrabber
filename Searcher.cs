...
namespace PhotoGrabber
{
    internal static class Searcher
    {
        ...
        public static async Task<string> searchMedia(string url, string folderName, int fileLimit, string fileType)
        {
                ...
                try
                {
                    ...
                    var Images = driver.FindElements(By.ClassName(className)); // Search for class
                    ...
                    for (int i = 0; i < Images.Count(); i++)
                    {
                        ...
                        Images = driver.FindElements(By.ClassName(className)); // For every
                        Images[i].Click();
                        Console.WriteLine("starting");
                        Thread.Sleep(1000); // Give each click time to load
                        var imageToSave = driver.FindElements(By.TagName(tagName)); // Now we look for the img tag and the source
                        ...
                        for (int j = 0; j < imageToSave.Count(); j++)
                        {
                            ...
                            Console.WriteLine(j + " | Viewing: " + imageURL + " | saving as :" + folderName + num + fileType); // Testing only, outputs currently viewing 
                            WebClient downloadCurImage = new WebClient();

                            // Generates a random number to create filename

                            downloadCurImage.DownloadFile(imageURL, destination + folderName + num + fileType);
                            iter++;
                            if (iter == fileLimit)
                            {
                                quitDriver();
